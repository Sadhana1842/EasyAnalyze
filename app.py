...
if uploaded_file:
    ...
    def calc_group_stats(dataframe, start_date, end_date, group_cols, weightage_metric):
        ...
        if weightage_metric == "TCR":
            ...
            grouped["Weightage (Sumproduct)"] = ((grouped["Sum of SurveyCount2"]/100)*grouped["CSAT%"]).round(4)
    ...
    if weightage_metric == "TCR":
        ...
        merged["Mix Shift Impact"] = (merged["CSAT% R1"] / 100) * merged["Sum of SurveyCount2 R2"]
        merged["Score Impact"] = (merged["Sum of SurveyCount2 R1"] / 100) * merged["CSAT% R2"]
            ...
            if row.name == len(multi_df):
                ...
                for i, col in enumerate(row.index):
                    ...
                    if 'Diff' in col:
                        ...
                        if pd.notna(val):
                            if val > 0:
                                styles[i] = 'font-weight: bold; background-color: #d4edda; color: #155724'
                            elif val < 0:
                                styles[i] = 'font-weight: bold; background-color: #f8d7da; color: #721c24'
                            else:
...
# Legends container - pushes to sidebar bottom
legends_container = st.sidebar.container()
with legends_container:
    st.markdown("---")
    with st.expander("📋 Column Legends", expanded=False):
        ...
        **Weightage**: Weighted Contribution of each group's TCR/CSAT <br> (SurveyCount2% × **TCR%** (or **CSAT%** based on toggle)) <br>
        
        
        **Mix Shift Impact**: Effect of Change in Survey Sample <br> (Metric_R1 × SurveyCount2%_R2) <br>
        
        
        **Score Impact**: Effect of Change in Score (TCR/CSAT) <br> (SurveyCount2%_R1 × Metric_R2) <br>
